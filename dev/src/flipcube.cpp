...
D3DTEXTUREBLEND  TextureBlend       = D3DTBLEND_COPY;
...
BOOL Init3DState()
{
     ...
     */
    d3dDevice->SetRenderState(D3DRENDERSTATE_ZENABLE, zflag);
    d3dDevice->SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, zflag);
...
BOOL DDSetMode(int width, int height, int bpp)
{
    ...
    if (bpp == 8)
    {
        ...
        GetPaletteEntries(GetStockObject(DEFAULT_PALETTE), 0,  10, &ape[0]);
        GetPaletteEntries(GetStockObject(DEFAULT_PALETTE), 10, 10, &ape[246]);
