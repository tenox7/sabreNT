...
public:
   ...
   void DeleteAttacker(unsigned long attackerIdx);
	...
	virtual void SetManeuver(int mnvr, unsigned long flags = 0, aiREAL d0 = 0.0,
						 aiREAL d1 = 0.0, aiREAL d2 = 0.0);
	virtual void SetManeuverX(int mnvr, unsigned long flags = 0, 
								  aiREAL d0 = 0.0, aiREAL d1 = 0.0, aiREAL d2 = 0.0,
								  aiREAL d3 = 0.0, aiREAL d4 = 0.0, aiREAL d5 = 0.0);
	...
	const aiManeuverState &GetManeuverStackTop();
	const aiManeuverState &GetManeuverStackBottom();
...
protected:
   ...
   aiREAL          timeFrame;                 /* time in secs from last update*/
   aiREAL          oneOverTimeFrame;
   aiREAL          elapsedTime;               /* total time elapsed           */
   int            logFlag;                   /* true if logging              */
   FILE           *logFile;                  /* logging file                 */

   aiManeuverState  mnvrStack[aiB_MNVR_MAX];  /* the maneuver stack           */
   int             mnvrStackPtr;             /* maneuver stack pointer       */
   aiManeuverState *curManeuver;              /* current maneuver             */

	/* attacker list */
	aiAttackerList	attackList;
	aiREAL				attackerCheckInterval;   /* time interval for checking attacker list */ 

	int				skillLevels[aiB_SKILL_SLOTS];

	void					ExecManeuverStack(void);
	virtual void		UpdateManeuver(aiManeuverState &im);
	aiManeuverState		*PushManeuver(int mnvr, unsigned long flags = 0, aiREAL d0 = 0.0,
												aiREAL d1 = 0.0, aiREAL d2 = 0.0);
	aiManeuverState		*PushManeuverX(int mnvr, unsigned long flags = 0,
												aiREAL d0 = 0.0, aiREAL d1 = 0.0, aiREAL d2 = 0.0,
												aiREAL d3 = 0.0, aiREAL d4 = 0.0, aiREAL d5 = 0.0);
	aiManeuverState		*PopManeuver(void);
