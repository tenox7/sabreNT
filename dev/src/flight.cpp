...
void Flight::calcState(float t)
{
  ...
  // Determine heading
  state.h_direction = Vector(state.flight_port.look_at.x -
...
void Flight::calcForces(float)
{
  ...
  forces.thrust.magnitude = specs->max_thrust *
    ...
    mods.engine_e *
    state.air_density;
  forces.thrust.direction = Vector(0,0,1);
  ...
  if (forces.lift.magnitude < 0)
    {
      ...
      forces.lift.direction = Vector(0,-1,0);
    }
  else
    forces.lift.direction = Vector(0,1,0);
  /* 
  ...
  forces.g.direction = Vector(0,0,-1);
...
int no_z = 0;
	...
	{
		float aX;
		Vector v = to_port(state.velocity.direction);
		aX = fabs(v.X);
		if (aX > 0.0)
		{
