...
void Debris::activate(R_3DPoint &p, Vector &dir)
{
	Projectile::activate(p,dir);
}
...
void Smoke_Trail::activate(Flight &flt)
{
  ...
  v = flt.state.velocity.direction;
  v *= -1;
  v.Normalize();
  v *= d_len;
  flight = &flt;
...
void Rocket_Trail::activate(FlightLight *flt)
{
  ...
  v = fltlte->state.velocity.direction;
  v *= -1;
  v.Normalize();
  v *= d_len;
...
void Unguided_Manager::initUnguided()
{
	if (the_umanager)
	{
		the_umanager->m_rounds.de_activate();
		the_umanager->m_flak.de_activate();
		the_umanager->m_blasts.de_activate();
		the_umanager->m_dustups.de_activate();
		the_umanager->m_debris.de_activate();
		the_umanager->m_smokes.de_activate();
		the_umanager->m_strails.de_activate();
	}
	else
		inited = 1;
  ...
  for (i=0;i<nrounds;i++)
    {
      ...
      rounds[i]->view_len = 3.0 * world_scale;
	...
	case FLIGHT_ZVIEW_T:
		{
			...
			dustup_size = (RANDOM(6) + 1.0) * world_scale;
			...
			if (fvwr->isHistory())
			{
				...
				if (vol > 0)
					sound_on(SNDGLOBAL_EXPLOSION_MEDIUM,NORM,vol);
				create_debris(hit_point,RANDOM(5) + 3, RANDOM(5) + 3, RANDOM(8) + 5,	vq);
				     ...
				     int n, Vector_Q *vq)
{
  ...
  */
  ...
  while (n--)
    {
      int x = RANDOM(vsize);
      debris_specs.time_limit = ((float)RANDOM(3)) + 1.0;
      debris_specs.weight = RANDOM(120) + 30;
      debris_specs.mass = debris_specs.weight / 32.0;
      debris_specs.init_speed = RANDOM(40) + 20;
      ...
      if (vq != NULL)
	{
	  v = Vector(vtable[x][0],vtable[x][1],
			      vtable[x][2]);
	  v.Normalize();
	  v *= (((float)RANDOM(1000)) + 500.0) * world_scale;
	  v = v + to_vector(*vq);
	  debris_specs.init_speed = v.Magnitude();
