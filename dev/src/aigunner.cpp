...
aiGunner::aiGunner(aiFlightModel *flightModel, int index,
					  unsigned long ownerIdx, int affiliation, aiREAL bulletRadius,
                 aiREAL gunPitchRate, aiREAL gunYawRate,
                 aiAttitude *jiggleAtt)
{
...
void aiGunner::Update(double timeFrame)
{
	...
	if (hasTarget)
	{
		...
		AdjustGun();
		Shoot();
		hasTarget = aiTarget::GetTargetActive(TARGET_IDX);
	}
}

void aiGunner::AdjustGun()
{
	aiREAL pitchStep,yawStep;
	aiREAL d;

	if (TARGET_RANGE > weaponLimits.maxRange)
		return;
	if (aiIsBetween(CONE_MIN_PITCH,CONE_MAX_PITCH,LEAD_PITCH))
	{
		d = aiGetCircularDistance(GUN_PITCH,LEAD_PITCH);
		pitchStep = gunPitchRate * timeFrame;
		if (pitchStep > fabs(d))
			pitchStep = fabs(d);
		if (d < 0)
			pitchStep = -pitchStep;
		GUN_PITCH = aiIncrementAngle(GUN_PITCH,pitchStep);
	}
	if (aiIsBetween(CONE_MIN_YAW,CONE_MAX_YAW,LEAD_YAW))
	{
		d = aiGetCircularDistance(GUN_YAW,LEAD_YAW);
		yawStep = gunYawRate * timeFrame;
		if (yawStep > fabs(d))
			yawStep = fabs(d);
		if (d < 0)
			yawStep = -yawStep;
		GUN_YAW = aiIncrementAngle(GUN_YAW,yawStep);
	}
}
...
int aiGunner::Shoot()
{
	...
	result = (GUN_LEAD_RANGE_SQ <= weaponLimits.maxRange * weaponLimits.maxRange &&
					aiIsBetween(CONE_MIN_PITCH,CONE_MAX_PITCH,LEAD_PITCH) &&
					aiIsBetween(CONE_MIN_YAW,CONE_MAX_YAW,LEAD_YAW));
...
void aiGunner::GetTargetGeometry()
{
	...
	aiTarget::GetTargetInfo(TARGET_IDX,targetPosition,targetVelocity,
						 targetHeading);
	...
	GUN_LEAD_RANGE_SQ = aiDistanceSquared(ourPosition,LEAD_POINT);
}
