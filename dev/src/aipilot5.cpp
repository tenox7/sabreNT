...
void aiPilot::DoLevelBomb(aiManeuverState &mv)
{
    aiREAL     timeToTarget;
    aiREAL     timeOfFall;
    aiVector   v;
 ...
 *****************************************************************/
void aiPilot::DoEvade(aiManeuverState &mv)
{
	...
	case BASE:
	{
		...
		else if (TARGET_FLAGS.forward)
		{
			...
			if (TARGET_YAW < 0.0)
				...
				turn_bits = AIMNVR_RIGHT;
			PushManeuver(aiManeuver::HARD_TURN,turn_bits);
			mv.state = TURN;
		}
		else
		{
			PushManeuver(aiManeuver::EXTEND,0,engageMaxSpeed,20.0,extAngle);
			mv.state = EXTEND;
		}
	}
		break;
