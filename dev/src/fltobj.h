...
public:
   ...
   * AND MORE! AND MORE!    *
	**************************/
  _aiPID pitchPid;					/* pid for pitch control */
  _aiPID rollPid;					/*         roll control  */
  _aiPID yawPid;               /*         yaw control   */
  _aiPID gPid;                 /*         g's control   */
  _aiPID airSpeedPid;          /*         air speed     */
  _aiPID targetClosurePid;     /*         target closure */
...
public:
  ...
  Vector_Q()
    : magnitude(0),
      direction(0,0,0)
  {}
  Vector_Q(float &mag, const Vector &v)
  ...
  Vector_Q &operator =(const Vector &v)
  {
  ...
  friend Vector to_vector(Vector_Q &vq);
  friend Vector_Q operator +(Vector_Q &, Vector_Q &);
  friend Vector operator +(Vector &, Vector_Q &);
...
public:
  ...
  Vector vel_pv;        /* direction of velocity in port coordinants */
  ...
  Vector h_direction;   /* Vector representing movement on xy plane */
