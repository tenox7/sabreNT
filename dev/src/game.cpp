...
const aiManeuverState &mvs = nde.pilot->GetManeuverStackTop();
	if (!nde.pilot->IsActive())
	{
		...
		result = 4;
	else if (mvs.maneuver == aiManeuver::GUN_ATTACK)
	{
		const aiTarget &tg = nde.pilot->GetEngageTarget();
		if (tg.flags.onNose)
			result = 5;
		else
			result = 4;
	}
	else if (mvs.maneuver == aiManeuver::PURSUIT)
		result = 3;
	else
	{
		const aiManeuverState &mvsbot =
			nde.pilot->GetManeuverStackBottom();
		if (mvsbot.maneuver == aiManeuver::ENGAGE)
		{
			if (mvsbot.state == 12)
				result = 6;
			else
...
void GameSession::update_flight()
{
  ...
  if (select_next > 0)
    {
      fm.select_next_target(fm.view_node,select_next);
      select_next = 0;
    }
}
...
//	show_message(0,"********** %d ************",um.m_debris.active_count);
	blit_buff();
	vwr.hitme = 0;
...
void __cdecl GameSession::show_message(int bot, char *str, ...)
{
  char buf[512];
  va_list ap;
