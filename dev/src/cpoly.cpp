...
void poly_params::read(istream &is)
{
  ...
  {
	 if (read_tags)
	   is >> color >> tmap >> flags >> color_range >> tag >> c;
	 else
...
void C_Poly::draw(Port_3D &port, int zclip)
{
	if (wpoints != NULL)
	{
...
#ifdef USES_DDRAW
			extern void wvgad3_rndrpoly(R_3DPoint *points, TxtPoint *tpoints, int np, 
			int fillColor, TextrMap *tmap, Port_3D &port);
			wvgad3_rndrpoly(wpoints,p_info->tpoints,p_info->npoints,
			color,tmap_ptr,port);
#else
...
#ifdef USES_DDRAW
		{
			extern void wvgad3_rndrpoly(R_3DPoint *points, TxtPoint *tpoints, int np, 
			int fillColor, TextrMap *tmap, Port_3D &port);
			TxtPoint dummyPoints[32];
			wvgad3_rndrpoly(wpoints,dummyPoints,p_info->npoints,color,NULL,port);
		}
#else
...
void C_Poly::set_shadow(REAL_TYPE , Vector &light_source)
{
...
void C_Poly::set_flag(unsigned long flag, int set)
{
	if (set)
		flags |= flag;
	else
...
C_Poly *C_Shape::find_poly_by_tag(int tag)
{
	C_Poly *result = NULL;
	for (int i=0;i<npolys;i++)
	{
...
void C_Shape::set_poly_flag_by_tag(unsigned long flag, int set, int tag)
{
	C_Poly *poly = find_poly_by_tag(tag);
	if (poly)
		poly->set_flag(flag,set);
}
