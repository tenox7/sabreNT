...
void SimFont::put_char(unsigned char c, int x, int y, int color, int opt)
{
  ...
  if (opt > 0)
	  set_option(opt);
  put_char(c,x,y,color,buffer_ptr,option);
  free_xbuff();
}
...
void __cdecl SimFont::font_sprintf(int x, int y, int color, int opt, char *format, ...)
{
  char buff[120];
  va_list ap;
...
void Font8x8::put_char(unsigned char c, int x, int y, int color,
								unsigned char *buffer_ptr, int dim)
{
	dim = 8;
...
void ConsoleFont::put_char(unsigned char c, int x, int y, int color,
			   unsigned char *, int )
#else
void ConsoleFont::put_char(unsigned char , int , int , int , unsigned char *, int )
#endif
{
#ifndef SABREWIN
