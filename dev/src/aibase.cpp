...
{
	this->timeFrame = (aiREAL) timeFrame;
...
int i;
	...
	while (i >= 0)
	{
		aiManeuverState &ms = mnvrStack[i];
		UpdateManeuver(ms);
...
// Get top of stack
const aiManeuverState & aiBase::GetManeuverStackTop()
{
	if (mnvrStackPtr >= 0)
...
// Get bottom of stack
const aiManeuverState & aiBase::GetManeuverStackBottom()
{
	return mnvrStack[0];
}
...
aiManeuverState *aiBase::PushManeuver(int mnvr, unsigned long flags, aiREAL d0,
												aiREAL d1, aiREAL d2)
{
...
aiManeuverState *aiBase::PushManeuverX(int mnvr, unsigned long flags,
													aiREAL d0, aiREAL d1, aiREAL d2,
													aiREAL d3, aiREAL d4, aiREAL d5)
{
	if (mnvrStackPtr < aiB_MNVR_MAX - 1)
	{
		mnvrStackPtr++;
...
aiManeuverState *aiBase::PopManeuver()
{
	if (mnvrStackPtr >= 0)
	{
		mnvrStackPtr--;
...
void aiBase::SetManeuver(int mnvr, unsigned long flags, 
									aiREAL d0, aiREAL d1, aiREAL d2)
{
	if (mnvr >= 1 && mnvr < aiManeuver::LAST)
	{
		ClearManeuverStack();
		PushManeuver(mnvr,flags,d0,d1,d2);
		LOG("Setting Maneuver To: %s",
			aiManeuver::Maneuver2String(mnvr));
	}
	else
...
void aiBase::UpdateManeuver(aiManeuverState &)
{
