...
int i;
int formationIndex;
aiPoint	 leaderPosition;
aiAttitude leaderAttitude;
	...
	if (NWaypoints() > 0)
	{
		...
		if (curWaypoint->task == swpTAKEOFF)
		{
			...
			aiField *airfield = aiField::GetaiField(fieldId);
			if (airfield)
			{
				aiRunway *runway = airfield->GetRunway(runwayNo);
				if (runway)
				{
					aiPoint		position;
					aiAttitude	attitude;
		...
		{
			aiVector v;
			aiPoint p = leaderPosition;
			p.z = curWaypoint->to.z;
			v = curWaypoint->to - p;
			aiVector2aiAttitude(v,leaderAttitude);
		...
		for (i=0;i<n;i++)
		{
			...
			att.roll = 0.0;
			aiFormationData::GetWingmanVector(curWaypoint->formationType,
						wPos++,
						formVect);
			formVect *= pil->GetFormationWingLen();
			aiPilot::BodyPoint2WorldPoint(formationIndex,(const aiPoint &)formVect,
					formPoint);
...
void aiFlite::InitFormation(aiWaypoint *wp)
{
...
int foundPlayer = 0;
	...
	if (HaaiAttackers() && mode == aiF_NAVIGATE && leader->HasGunners())
		DistributeAttackers();
	damageFlag = 0;
}
...
void aiFlite::DistributeAttackers()
{
  ...
  for (i=0;i<GetCount();i++)
    {
      aiPilot *pil = GetPilot(i);
      if (pil->IsActive() && !pil->HaaiAttackers())
	freePilots.Add(pil);
    }
	  ...
	  if (pil->IsActive() && pil->HaaiAttackers())
	    {
	      ...
	      aiAttacker *attkr = pil->GetFirstAttacker();
	      while (attkr)
		{
  ...
  if (targetMode == aiF_TARGETMODE_NOACTION)
    {
      if (!leader->IsPlayer())
	{
	  aiREAL dist = leader->GetNavigationDistance();
	  if (dist <= 1000.0)
	    {
...
{
  if (curWaypoint != NULL &&	aiGetElapsedTime() < curWaypoint->time)
    {
      for (int i=0;i<GetCount();i++)
...
void aiFlite::DoTargetaiWaypoint()
{
...
void aiFlite::DoCapWaypoint()
{
  ...
  aiREAL  engageRanges[aiMAX_FLITES];
  ...
  for (i=0;i<GetFliteCount();i++)
	  ...
	  && flite->IsActive())
	{
	  ...
	  aiREAL d = aiDistanceSquared(ourPos,itsPos);
...
void aiFlite::SetWaypoints(const aiWaypoint *wps, int nn)
{
  waypoints.Destroy();
  waypoints.Create(nn,1);
  for (int i=0;i<nn;i++)
    {
      aiWaypoint *wp = new aiWaypoint(wps[i]);
      waypoints.Add(wp);
      wp->next = NULL;
      if (i > 0 && i < nn - 1)
...
void aiFlite::SetWaypoints(const aiwaypoint_info *winfo, int nn)
{
  waypoints.Destroy();
  waypoints.Create(nn,1);
  for (int i=0;i<nn;i++)
    {
      aiWaypoint *wp = new aiWaypoint(winfo[i]);
      waypoints.Add(wp);
      wp->next = NULL;
      if (i > 0 && i < nn - 1)
	GetWaypoint(i-1)->next = wp;
...
aiWaypoint *aiFlite::GetWaypoint(int idx)
{
  if (idx >= 0 && idx < waypoints.Count())
    return ((aiWaypoint *) waypoints[idx]);
  else
    return (NULL);
}

aiWaypoint *aiFlite::GetNextWaypoint()
{
  wpidx++;
...
aiWaypoint *aiFlite::GetPreviouaiWaypoint()
{
  wpidx--;
  if (wpidx < 0)
    wpidx = 0;
...
void aiFlite::SetManeuver(int maneuver, long flags, aiREAL d0,
			  aiREAL d1, aiREAL d2)
{
  for (int i=0;i<GetCount();i++)
    {
...
void aiFlite::SetFormationWaypoint(aiWaypoint *wp, int leaderIndex)
{
  unsigned long targetIdx;
  SetWaypoints(wp,1);
  if (leaderIndex < 0)
...
{
  unsigned long	idx = 0;	
  aiREAL	minDistance = visualEngagementRadius + 100.0;
  int foundOne = 0;
...
void aiFlite::ProtectPlayer()
{
  ...
  if (aiPilot::playerPilot != NULL)
      ...
      if (IsPlayerFlite())
	{
	  ...
	  int pilIndex = 0;
	  aiAttacker *attkr = aiPilot::playerPilot->GetFirstAttacker();
	  if (!attkr)
...
void aiFlite::PreviouaiWaypoint()
{
  curWaypoint = GetPreviouaiWaypoint();
  InitFormation(curWaypoint);
}
				       ...
				       aiPilot *playerPilot)
{
  ...
  aiPoint			playerPosition;
  aiAttitude		playerAttitude;
  int				oClock;
  aiREAL				convYaw;
  char				*hiLo;
  char				distance[32];
 ...
 *******************************************/
aiObjectArray aiFlite::aiFlites(aiMAX_FLITES,aiFLITE_DEFAULT_OWNERSHIP);
