...
Flight_Specs::Flight_Specs()
{
    ...
    flags = 0;
	 default_aiPID(&pitchPid);
	 default_aiPID(&yawPid);
	 default_aiPID(&rollPid);
	 default_aiPID(&gPid);
...
Vector operator +(Vector_Q &vq, Vector &v)
{
  Vector v1 = vq.direction * vq.magnitude;
  return Vector(v + v1);
}
...
// Convert magnitudes
float r_mag;
Vector v1 = vq1.direction * vq1.magnitude;
Vector v2 = vq2.direction * vq2.magnitude;
