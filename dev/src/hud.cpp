...
Pilot &pilot = *my_node.pilot;
	...
	else
	{
		...
		if (my_flight.controls.autopilot)
		{
			...
			{
				const aiManeuverState &mvs = pilot.GetManeuverStackTop();
				pp = ((aiManeuverState &)mvs).GetManeuverString(); 
			}
		}
		else
			pp = "";
	...
	{
		R_3DPoint navPoint;
		const aiNavInfo &navInfo = pilot.GetNavInfo();
		aiPoint2R_3DPoint(navInfo.waypoint.to,navPoint);
...
void Hud::do_hud_target_flags(int left, int y, const aiTargetFlags &flgs)
{
char flagStr[32];
int  x;
...
void Hud::do_hud_formation_geometry(const aiTargetGeometry &tg, Port_3D &port)
{
R_3DPoint w,p;
int x,y,x1,y1;
char range_str[32];
char speed_str[32];
	...
	y = SCREEN_HEIGHT - 28;
	x = SCREEN_WIDTH - 7 * 6;
	aiPoint2R_3DPoint(tg.formationPoint,w);
	port.world2port(w,&p);
...
void Hud::do_hud_target_geometry(const aiTargetGeometry &tg, Port_3D &port)
{
int x,y;
char range_str[32];
...
Port_3D *fport = &my_flight.state.flight_port;
	...
	{
		Vector v = to_vector(my_flight.state.velocity);
		vr = R_3DPoint(v);
