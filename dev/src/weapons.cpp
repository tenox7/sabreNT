...
void Gun_Specs::write(ostream &os)
{
  ...
  os << rounds_per_second << ' ' << tracer_color << '\n';
  os << tracer_freq << '\n';
  os << ')' << '\n';
}
  ...
  if (n > 0)
    {
      ...
      for (int i=0;i<n;i++)
		{
			is >> positions[i] >> vectors[i] >> rounds;
...
void Weapon::get_launch_params(Launch_Params &lp)
{
	...
	if (wep_i->flags & WP_SETVIEW)
	{
		...
		v.Normalize();
		lp.launcher_velocity.direction = Vector(v);
	}
	else
...
R_3DPoint Weapon::predict_path (Flight &hf, float flt_time)
{
  ...
  Vector hv = to_vector(flt.state.velocity);
  float v_vel = hv.Z;
  ...
  if (wep_i->elapsed_time >= gun_specs->rounds_per_second)
    {
	  for (i=0;i<burst_steps;i++)
	  {
		...
		if (((wep_i->rounds_remaining + idx) % gun_specs->tracer_freq) == 0)
			lp.flags |= LP_TRACER;
		if (um->new_round(lp))
			{
				result++;
...
void Weapon_Instance::set_weapon(Weapon *wp)
{
  ...
  if (viewers != NULL)
    delete viewers;
  rounds_remaining = weapon->getTotalRounds();
