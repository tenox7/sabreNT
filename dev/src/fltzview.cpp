...
void Flight_ZViewer::InitTaggedPolys(void)
{
#ifdef SABRE_TAGS
	for (int i=0;i<nshapes;i++)
	{
		shapes[i].set_poly_flag_by_tag(INVISIBLE,1,GROUND_PLANE_TAG);
		shapes[i].set_poly_flag_by_tag(INVISIBLE,1,ENG1BLADE_TAG);
		shapes[i].set_poly_flag_by_tag(INVISIBLE,1,ENG2BLADE_TAG);
		shapes[i].set_poly_flag_by_tag(INVISIBLE,1,ENG3BLADE_TAG);
		shapes[i].set_poly_flag_by_tag(INVISIBLE,1,ENG4BLADE_TAG);
		for (int j=132;j<=144;j++)
			shapes[i].set_poly_flag_by_tag(INVISIBLE,1,j);
	}
#endif
}

void Flight_ZViewer::init(Z_Node_Manager *zm)
{
	Z_Viewer::init(zm);
}
