...
Vector FlightLight::to_port(Vector &v)
{
  R_3DPoint r_w,r_p;
  r_w = v.to_R_3DPoint();
...
Vector FlightLight::to_world(Vector &v)
{
  R_3DPoint r_w,r_p;
  r_p = v.to_R_3DPoint();
...
void FlightLight::activate(Flight &launcher)
{
  ...
  Vector v = launcher.state.flight_port.calc_view_normal();
			   ...
			   R_3DPoint &p1, Vector &v, Vector_Q *vq)
{
  ...
  Vector_Q vq1(specs->init_speed,v1);
  if (vq)
...
void FlightLight::add_extra_forces(const Vector &v)
{
	xtra = v;
}