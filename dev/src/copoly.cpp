...
int C_Oriented_Poly::set_visible(Port_3D &port)
{
	if (flags & INVISIBLE)
		visible = 0;
	else if (flags & NO_BACKFACE)
		visible = 1;
	else
	{
      if (O_CheckPlaneEquation(Vector(port.look_from),
			       surface_normal,
			       plane_constant))
			visible = 1;
      else
...
void C_Oriented_Shape::set_visible(Port_3D &port)
{
  int i;
...
C_Oriented_Poly * C_Oriented_Shape::find_poly_by_tag(int tag)
{
	C_Oriented_Poly *result = NULL;
	for (int i=0;i<npolys;i++)
	{
		if (o_polys[i].tag == tag)
		{
			result = &o_polys[i];
			break;
		}
...
void C_Oriented_Shape::set_poly_flag_by_tag(unsigned long flag, int set, int tag)
{
	for (int i=0;i<npolys;i++)
		if (o_polys[i].tag == tag)
			o_polys[i].set_flag(flag,set);
}
