...
void Flight_Node::read(istream &is)
{
	...
	if (do_random)
	{
		...
		Vector v;
		GetRandomVector(&v,1000,10000,1000,10000,0,2000);
		v *= world_scale;
...
void Flight_Manager::readNodes(istream &is)
{
	...
	for (i=0;i<n_flights;i++)
	{
		...
		is >> *(flight_nodes[i]);
		flight_nodes[i]->pilot->set_node_index(i);
	}
}
...
aiwaypoint_info waypoints[MAX_SWAYPOINT_INFOS];
int nwaypoints;
int nflites;
int npilots;
...
static simfileX::dict frmtntype[NFRMTNTYPES] =
{
	{ "wedge", aifWEDGE },
	{ "vic", aifVIC },
	{ "echelon_right", aifECHELON_R },
	{ "echelon_left", aifECHELON_L },
	{ "trail", aifTRAIL },
	{ "ladder", aifLADDER },
	{ "diamond", aifDIAMOND },
	{ "fighting_wing", aifFIGHTING_WING }
};
#define NMNVRTYPES 16
static simfileX::dict mnvrtypes[NMNVRTYPES] =
{
	{ "straight_and_level", aiManeuver::STRAIGHT_AND_LEVEL },
	{ "standard_turn", aiManeuver::STANDARD_TURN },
	{ "climb", aiManeuver::CLIMB },
	{ "descend", aiManeuver::DESCEND },
	{ "break_turn", aiManeuver::BREAK_TURN },
	{ "hard_turn", aiManeuver::HARD_TURN },
	{ "immelman", aiManeuver::IMMELMAN },
	{ "split_s", aiManeuver::SPLIT_S },
	{ "jink", aiManeuver::JINK },
	{ "wingover", aiManeuver::WINGOVER },
	{ "climbing_turn", aiManeuver::CLIMBING_TURN },
	{ "snap_roll", aiManeuver::SNAP_ROLL },
	{ "chandelle", aiManeuver::CHANDELLE },
	{ "zoom", aiManeuver::ZOOM },
	{ "barrel_roll", aiManeuver::BARREL_ROLL },
	{ "spiral_dive", aiManeuver::SPIRAL_DIVE }
};
	...
	for (i=0;i<nflites;i++)
	{
		...
		if (nwaypoints > 0)
		{
			...
			{
				READ_TOKI('{',is,c);
				aiwaypoint_info &waypoint_info = waypoints[j];
		      ...
		      simfileX::readdictinput(is,waypointTaskStr,sizeof(waypointTaskStr),
				...
				if (waypoint_info.formation_type < aifWEDGE)
					waypoint_info.formation_type = aifWEDGE;
				if (waypoint_info.formation_type > aifFIGHTING_WING)
					waypoint_info.formation_type = aifFIGHTING_WING;
...
int Flight_Manager::select_next_target(int node, int mode)
{
	int i;
	int ii = flight_nodes[node]->pilot->get_target_node_index()+1;
	int found = -1;

	i=0;
	while (i++ < n_flights && found < 0)
	{
		if (ii >= n_flights)
			ii = 0;
		if (	ii != node &&
				flight_nodes[node]->pilot->get_target_node_index() !=	ii && 
				!flight_nodes[ii]->flight->state.crashed )
		{
			switch (mode)
			{
			case NEXT_ENEMY:
				if (flight_nodes[node]->pilot->GetAffiliation() != 
					 flight_nodes[ii]->pilot->GetAffiliation())
						found = ii;
			break;
			...
			case NEXT_WINGMAN:
				if (flight_nodes[ii]->pilot->GetFliteId() ==
					 flight_nodes[node]->pilot->GetFliteId())
					 found = ii;
			break;
	...
	if (found > 0)
		flight_nodes[node]->pilot->set_target(flight_nodes[found]->pilot);
