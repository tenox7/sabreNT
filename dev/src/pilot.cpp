    ...
    sbrFlight(flt)
{
	...
	new_damage = 0.0;
	node_index = target_node_index = 0;
	...
	/*
	 *  Build list of guns from
	 *  loaded weapons
	 */
	nguns = 0;
	for (i=0;i<n_weaps;i++)
	{
		if (weapons[i].getType() == gun_t)
		{
			if (nguns < sbrPIL_MAX_GUNS)
			{
				guns[nguns] = i;
				gunsRounds[nguns++] = aiRand(sbrPIL_SHOTS_PER_WEAPON_MIN,
													 sbrPIL_SHOTS_PER_WEAPON_MAX);
			}
			else
				break;
		}
...
void Pilot::init()
{
	...
	SetTask(NOTHING);
	SetManeuver(aiManeuver::STRAIGHT_AND_LEVEL,AIMNVR_LOOPBIT);
	grndColSecs = 5.0;
...
void Pilot::update(int aiActive)
{
	...
	if (!aiActive && task == ENGAGE_TARGET && TARGET_ACTIVE && target_pilot == NULL)
	{
		...
		if (pil && target_node_index != ((Pilot*)pil)->get_node_index())
...
void Pilot::set_target(Pilot *pl)
{
	...
	if (target_pilot != NULL)
	{
		...
		target_node_index = pl->get_node_index();
	}
	else
	{
		target = NULL;
...
void Pilot::checkForNextWeapon()
{
	if (gunsRounds[gunsIdx] <= 0)
	{
		gunsRounds[gunsIdx++] = aiRand(sbrPIL_SHOTS_PER_WEAPON_MIN,
													 sbrPIL_SHOTS_PER_WEAPON_MAX);
		if (gunsIdx >= nguns)
			gunsIdx = 0;
	}
	selWeapon(guns[gunsIdx]);
}
...
void Pilot::GetWeaponLimits(aiWeaponLimits &weaponLimits)
{
FlightLight_Specs *flspecs;
	...
	weaponLimits.burstTime = 0.4 + aiRandPer() * 0.4;
	weaponLimits.burstPauseTime = 0.1 + aiRandPer() * 0.2;
...
void Pilot::CalcGunLeadPoint(aiTargetGeometry &tg)
{
REAL_TYPE t;
aiVector   temp;
aiPoint    leadPoint;
...
aiPoint	 gunPoint;
aiAttitude ourAtt;
aiAttitude gunPointAtt;
aiAttitude leadPointAtt;
	...
	R_3DPoint2aiPoint(gun_point,gunPoint);
	GetAttitude(gunPoint,temp,gunPointAtt);

	aiPoint2R_3DPoint(tg.worldPosition,sel_weapon->target_position);
	...
	aiPoint2R_3DPoint(tg.worldPosition,track_point);
	aiPoint2R_3DPoint(tg.leadPoint,lead_track_point);
	aiPoint2R_3DPoint(tg.gunLeadPoint,gun_track_point);
}
...
void Pilot::Shoot()
{
	...
	if (InShootParams())
	{
		...
		if (gunsGunsSoundTime <= 0.0)
		{
			...
			gunsGunsSoundTime = aiRandPer() * GUNSGUNSPAUSE + 1.0;
		}
	}
}
...
{
int priorityValue=sbrPRIORITY_REGULAR;
int affil;
