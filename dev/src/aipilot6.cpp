 ...
 * TakeOff                                                   *
 *************************************************************/
void aiPilot::DoTakeOff(aiManeuverState &mv)
{
	aiVector		v;
	...
	case 4:
		...
		if (flightModel->GetHeightAGL() > 100.0)
		{
			...
			PushManeuver(aiManeuver::CLIMB,0,mv.data0,100.0);
			mv.state = 5;
		}
		break;
 ...
 * Landing                                                   *
 *************************************************************/
void aiPilot::DoLand(aiManeuverState &mv)
{
	EnableGroundCollision(0);
	/*	
	switch (mv.state)
	{
	case 0:
		PushManeuver(aiManeuver::STANDARD_TURN,0,aiPILOT_CIRCLE_BANK_DEG,
						 standardTurnSpeedMPH);
		mv.state = 1;
		break;
	...
	case 2:
		{
			...
			if (airfield)
			{
				...
				if (runway)
				{
					...
					aiREAL turnSpeedFPS = sMPH2FPS(standardTurnSpeedMPH);
					tsSq = turnSpeedFPS * turnSpeedFPS;
					t0 *= 32.2;
					aiREAL turnRadius = tsSq / t0;
					...
					aiVector2aiAttitude(w1,attitude);
					attitude.yaw += beta;
					attitude.pitch = attitude.roll = 0.0;
					v0.y = 1.0;					
					aiMatrix3Build(attitude,mat);
					aiMatrix3Rotate(mat,v0,w0);
					w0 *= dE;
			...
			PushManeuver(aiManeuver::NAVIGATE);
			mv.state = 3;
		}
		break;
 ...
 * Taxiing                                                  *
 ************************************************************/
void aiPilot::DoTaxi(aiManeuverState &mv)
{
	EnableGroundCollision(0);
}
