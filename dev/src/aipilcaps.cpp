...
aiObjectArray aiPilotCaps::aiPilotCapsArray(aiMAXPILOTCAPS,1);

void aiPilotCaps::AddaiPilotCaps(aiPilotCaps &pilotCaps)
{
	aiPilotCapsArray.Add(&pilotCaps);
}

void aiPilotCaps::AddaiPilotCaps(aipilot_caps *caps)
{
    if (aiPilotCapsArray.Count() < aiMAXPILOTCAPS)
    {
       aiPilotCaps *pilotCaps = new aiPilotCaps(*caps);
		 AddaiPilotCaps(*pilotCaps);
    }
}

aiPilotCaps *aiPilotCaps::GetaiPilotCaps(const char *capsId)
{
    aiPilotCaps *result = NULL;
    for (int i=0;i<aiPilotCapsArray.Count();i++)
    {
      aiPilotCaps *pilotCaps = (aiPilotCaps *)aiPilotCapsArray[i];
...
void aiPilotCaps::FlushaiPilotCaps()
{
	aiPilotCapsArray.Flush();
}

aiPilotCaps::aiPilotCaps(const aipilot_caps &caps)
{
